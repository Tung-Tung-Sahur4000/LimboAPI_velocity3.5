...
> **Velocity-CTD fork (separate build).** This is a dedicated build of LimboAPI for
> [Velocity-CTD](https://github.com/GemstoneGG/Velocity-CTD), maintained separately from the
> normal-Velocity version. It is compiled against the `com.velocityctd` artifacts from
> `https://repo.velocityctd.com/snapshots/` and uses CTD's login/join and connection methods
> (2-arg `LoginEvent`, future-based `registerConnection`, `markLoginEventFired`/`finalizeLogin`,
> `VelocityServer` tab lists). Because those join/connection paths differ from vanilla, this jar
> **does not run on normal Velocity** — use the upstream
> [Elytrium/LimboAPI](https://github.com/Elytrium/LimboAPI) for that. The CTD jar is published as
> `limboapi-ctd-<version>.jar` so it is not confused with a vanilla build.
